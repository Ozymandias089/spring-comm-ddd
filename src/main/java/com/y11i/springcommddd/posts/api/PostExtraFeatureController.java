...
public class PostExtraFeatureController {
    ...
    // ---------------------------------------------------------------------
    // 4. 신고 & 차단 (Moderation 1차 기능 스텁)
    // ---------------------------------------------------------------------

    /**
     * 게시글 신고.
     * <p>
     * 예: POST /api/posts/{postId}/report
     */
    @PostMapping(path = "/posts/{postId}/report", consumes = "application/json")
    @ResponseStatus(HttpStatus.NOT_IMPLEMENTED)
    public void reportPost(
            @AuthenticatedMember MemberId reporterId,
            @PathVariable String postId,
            @RequestBody(required = false) Object body // 추후 ReportRequestDTO로 교체
    ) {
        throw new UnsupportedOperationException("moderation.report.post: Not supported yet.");
    }

    /**
     * 사용자 차단.
     * <p>
     * 예: POST /api/members/{memberId}/block
     */
    @PostMapping(path = "/members/{memberId}/block")
    @ResponseStatus(HttpStatus.NOT_IMPLEMENTED)
    public void blockMember(
            @AuthenticatedMember MemberId me,
            @PathVariable String memberId
    ) {
        throw new UnsupportedOperationException("moderation.block.member: Not supported yet.");
    }

    /**
     * 사용자 차단 해제.
     * <p>
     * 예: DELETE /api/members/{memberId}/block
     */
    @DeleteMapping(path = "/members/{memberId}/block")
    @ResponseStatus(HttpStatus.NOT_IMPLEMENTED)
    public void unblockMember(
            @AuthenticatedMember MemberId me,
            @PathVariable String memberId
    ) {
        throw new UnsupportedOperationException("moderation.unblock.member: Not supported yet.");
    }
}
