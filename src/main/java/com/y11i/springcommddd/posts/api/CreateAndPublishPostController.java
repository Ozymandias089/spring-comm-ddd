...
public class CreateAndPublishPostController {
    ...
    /**
     * 게시글을 생성하고 즉시 게시(PUBLISHED) 상태로 만든다.
     *
     * <p>
     * type 에 따라 TEXT / LINK / MEDIA 중 하나로 동작하며,
     * 요청 스키마는 Draft 생성과 동일하게 사용한다.
     * </p>
     */
    @PostMapping(path = "create/publish", consumes = "application/json", produces = "application/json")
    @ResponseStatus(HttpStatus.CREATED)
    public PostDraftCreatedResponseDTO createAndPublish(
        ...
        // 3. 유스케이스 호출
        PostId postId = createAndPublishPostUseCase.createAndPublish(
                new CreateAndPublishPostUseCase.CreateAndPublishCommand(
